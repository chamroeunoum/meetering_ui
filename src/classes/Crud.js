/**
 * Crud Class — Client-side CRUD state machine for a single model
 *
 * Encapsulates all CRUD operations (list, read, create, update, delete, upload,
 * toggleActive) for a given API model. Manages local records, pagination, and
 * selected/matched state so UI components don't need to track these independently.
 *
 * Key responsibilities:
 *   - Tracks the server URL, model config, columns, records, and pagination
 *   - Generates pagination button arrays for the UI
 *   - Provides callback-based async CRUD methods with built-in error handling
 *   - Stores nested record arrays: all, selected, matched, created, updated, deleted
 *
 * @class Crud
 * @module classes/Crud
 */
...
class Crud {
  ...
  setModel(model) {
    ...
    if (typeof model === "object" && model.module != undefined) {
      this._model = model;
    } else {
  ...
  setTotalButtons(numberOfButotns = 5) {
    if (!(typeof numberOfButotns === "number")) {
      throw new Error(
        `ចំនួន គ្រាប់ចុច មិនត្រឹមត្រូវ ៖ ${numberOfButotns} មិនមែនជាប្រភេទតួលេខ។`,
      );
    }
  ...
  generateButtons() {
    this._pagination.start =
      this._pagination.page - parseInt(this._pagination.totalButtons / 2);
    this._pagination.start =
      this._pagination.start <= 0 ? 1 : this._pagination.start;
    this._pagination.end =
      this._pagination.page + parseInt(this._pagination.totalButtons / 2);
    this._pagination.end =
      this._pagination.end >= this._pagination.totalPages
  ...
  setPerPage(perPage) {
    if (!(typeof perPage === "number")) {
      throw new Error(
        `ចំនួនទិន្នន័យក្នុង ១ ទំព័រ មិនត្រឹមត្រូវ ៖ ${perPage} មិនមែនជាប្រភេទតួលេខ។`,
      );
    }
    ...
    try {
      if (params != null && params != undefined) {
        ...
        if (paramsArrayKeys.indexOf("page") != -1) {
          this.setPage(parseInt(params.page) > 0 ? parseInt(params.page) : 0);
        }

        if (paramsArrayKeys.indexOf("perPage") != -1) {
          this.setPerPage(
            parseInt(params.perPage) > 0 ? parseInt(params.perPage) : 0,
          );
        }
  ...
  async read(id, callback = () => {}, errorCallback = () => {}) {
    ...
    try {
      if (
        this._records.record != undefined &&
        parseInt(this._records.record.id) > 0
      ) {
        return await crud
          .read(
            this._server +
        ...
        errorCallback({ message: `មិនមានព័ត៌មានឡើយ ៖ ${id}` });
      }
    } catch (error) {
      errorCallback(error);
      console.error(error);
  ...
  async create(params, callback = () => {}, errorCallback = () => {}) {
    ...
    try {
      let paramsArrayKeys = Object.keys(params);
      if (paramsArrayKeys.length < 1)
        throw new Error(`អថេមិនត្រឹមត្រូវ ៖ ${params}`);

      return await crud
        .create(this._server + "/" + this._model.module + "/create", params)
        .then((res) => {
          if (res.data.ok == true) {
            // Write to localStorage with specific model name
            this._records.record = res.data.record;
  ...
  async update(params, callback = () => {}, errorCallback = () => {}) {
    ...
    try {
      ...
      return await crud
        .update(this._server + "/" + this._model.module + "/update", params)
        .then((res) => {
          if (res.data.ok == true) {
            // Write to localStorage with specific model name
            this._records.record = res.data.record;
  ...
  async upload(id, formData, callback = () => {}, errorCallback = () => {}) {
    ...
    try {
      if (
        ...
        return await crud
          .upload(this._server + "/" + this._model.module + "/upload", formData)
          .then((res) => {
            if (res.data.ok == true) {
              // Write to localStorage with specific model name
              this._records.record = res.data.record;
  ...
  getDetails() {
    console.log(`model: ${JSON.stringify(this._model)}`);
  }
  ...
  async next(callback, errorCallback = () => {}) {
    this._pagination.page = this._pagination.page >= this._pagination.totalPages
      ? this._pagination.totalPages
