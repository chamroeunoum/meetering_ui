<template>
  <div class="meeting-detail-page font-ktr bg-gray-50">
    <Transition name="fade">
      <div v-if="loading" class="flex items-center justify-center h-64">
        <div class="spinner text-center">
          ...
          <br/><br/><span class="text-gray-500">កំពុងអាន...</span>
        </div>
      </div>
    </Transition>
    <Transition name="fade">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6 ">
          <div class="flex flex-wrap items-start justify-between mb-4">
            <div class="flex-1 min-w-0 mr-4">
              <h1 class="font-moul text-xl text-gray-800 leading-relaxed">{{ record.objective || 'គ្មានចំណងជើង' }}</h1>
            </div>
            <div class="flex-none">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="flex items-start space-x-2"><svg class="w-5 h-5 text-blue-500 flex-none mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M17.75 3A3.25 3.25 0 0 1 21 6.25v11.5A3.25 3.25 0 0 1 17.75 21H6.25A3.25 3.25 0 0 1 3 17.75V6.25A3.25 3.25 0 0 1 6.25 3h11.5zm0 1.5H6.25A1.75 1.75 0 0 0 4.5 6.25v11.5c0 .966.784 1.75 1.75 1.75h11.5a1.75 1.75 0 0 0 1.75-1.75V6.25a1.75 1.75 0 0 0-1.75-1.75zM7.75 7a.75.75 0 0 1 .75.75V9.5h7V7.75a.75.75 0 0 1 1.5 0V9.5h.75c.966 0 1.75.784 1.75 1.75v1.5a.75.75 0 0 1-1.5 0v-1.5a.25.25 0 0 0-.25-.25h-.75v.75a.75.75 0 0 1-1.5 0v-.75h-7v.75a.75.75 0 0 1-1.5 0v-.75H7.25a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h4a.75.75 0 0 1 0 1.5h-4A1.75 1.75 0 0 1 5.5 18.75v-7.5c0-.966.784-1.75 1.75-1.75H8V7.75A.75.75 0 0 1 8.75 7h-1z" fill="currentColor"></path></g></svg><div><div class="text-xs text-gray-500">កាលបរិច្ឆេទ</div><div class="font-bold text-gray-700">{{ formatDate(record.date) }}</div></div></div>
            <div class="flex items-start space-x-2"><svg class="w-5 h-5 text-blue-500 flex-none mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12S6.477 2 12 2zm0 1.5a8.5 8.5 0 1 0 0 17a8.5 8.5 0 0 0 0-17zM11.75 6a.75.75 0 0 1 .75.75V12h3.75a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1-.75-.75V6.75a.75.75 0 0 1 .75-.75z" fill="currentColor"></path></g></svg><div><div class="text-xs text-gray-500">ម៉ោង</div><div class="font-bold text-gray-700">{{ record.start }} - {{ record.end }}</div></div></div>
            <div class="flex items-start space-x-2"><svg class="w-5 h-5 text-blue-500 flex-none mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18 13c0-.55-.45-1-1-1h-4c-.55 0-1 .45-1 1v4c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1l1.27.67c.33.18.73-.06.73-.44v-2.46c0-.38-.4-.62-.73-.44L18 14v-1zm-7.2-9.1l-6 4.5c-.5.38-.8.97-.8 1.6v9c0 1.1.9 2 2 2h13c.55 0 1-.45 1-1s-.45-1-1-1H6v-9l6-4.5l6 4.5v1h2v-1c0-.63-.3-1.22-.8-1.6l-6-4.5a2.01 2.01 0 0 0-2.4 0z" fill="currentColor"></path></svg><div><div class="text-xs text-gray-500">បន្ទប់ប្រជុំ</div><div class="font-bold text-gray-700">{{ record.rooms && record.rooms.length > 0 ? record.rooms.map(r => r.name).join(', ') : 'មិនបានកំណត់' }}</div></div></div>
            <div class="flex items-start space-x-2"><svg class="w-5 h-5 text-blue-500 flex-none mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M12 2a4 4 0 0 1 4 4a4 4 0 0 1-4 4a4 4 0 0 1-4-4a4 4 0 0 1 4-4zm0 1.5a2.5 2.5 0 1 0 0 5a2.5 2.5 0 0 0 0-5zM2 19v-.5C2 14.693 5.907 13 12 13s10 1.693 10 5.5v.5a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2zm1.5 0a.5.5 0 0 0 .5.5h16a.5.5 0 0 0 .5-.5v-.5c0-2.003-3.358-4-8.5-4s-8.5 1.997-8.5 4v.5z" fill="currentColor"></path></g></svg><div><div class="text-xs text-gray-500">ក្រសួង ស្ថាប័ន</div><div class="font-bold text-gray-700">{{ record.organizations && record.organizations.length > 0 ? record.organizations.map(o => o.name).join(', ') : 'មិនបានកំណត់' }}</div></div></div>
          </div>
          <div v-if="record.route" class="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
            <div class="flex items-start space-x-2">
              <svg class="w-5 h-5 text-green-600 flex-none mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M12 2c3.196 0 6.348 1.116 8.35 3.855C22.235 8.425 23 11.268 23 14c0 6.075-11 8-11 8S1 20.075 1 14c0-2.732.765-5.575 2.65-8.145C5.652 3.116 8.804 2 12 2zm0 1.5c-2.804 0-5.598 1.009-7.35 3.395C3.015 9.144 2.5 11.615 2.5 14c0 4.325 7.51 6.25 9.223 6.484L12 20.5c1.686-.02 8.5-1.96 8.5-6.5 0-2.385-.515-4.856-2.15-7.105C16.598 4.509 13.804 3.5 12 3.5zM12 7a3.5 3.5 0 1 1 0 7a3.5 3.5 0 0 1 0-7zm0 1.5a2 2 0 1 0 0 4a2 2 0 0 0 0-4z" fill="currentColor"></path></g></svg>
              <div>
                <div class="text-xs text-green-600 font-bold mb-1">ផ្លូវទៅកាន់ទីតាំងប្រជុំ</div>
                <p class="text-sm text-gray-700 leading-relaxed">{{ record.route }}</p>
              </div>
            </div>
          </div>
          <div v-if="meetingLeaders.length > 0" class="mt-4 pt-4 border-t border-gray-100">
            <div class="flex items-start space-x-2"><svg class="w-5 h-5 text-yellow-500 flex-none mt-0.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M12 2a4 4 0 1 0 0 8a4 4 0 0 0 0-8zM6 7a6 6 0 1 1 12 0A6 6 0 0 1 6 7zM3.5 19v-.5C3.5 15.693 7.307 14 12 14s8.5 1.693 8.5 4.5v.5a1 1 0 0 1-1 1h-15a1 1 0 0 1-1-1zm1.5 0h14v-.5c0-2.003-4.358-3-7.5-3S5 16.496 5 18.5v.5z" fill="currentColor"></path></g></svg><div><div class="text-xs text-gray-500">អ្នកដឹកនាំប្រជុំ</div><div class="font-bold text-gray-700"><span v-for="(lm,i) in meetingLeaders" :key="i"><template v-if="lm.member&&lm.member.officers&&lm.member.officers.length">{{ lm.member.officers.map(o=>(o.countesy?o.countesy.name+' ':'')) }}{{ lm.member.lastname }} {{ lm.member.firstname }}</template><template v-else>{{ lm.member?lm.member.lastname+' '+lm.member.firstname:'' }}</template>{{ i<meetingLeaders.length-1?', ':'' }}</span></div></div></div>
          </div>
        </div>
          ...
          <n-tabs type="segment" animated>
            ...
            <n-tab-pane name="agenda" tab="របៀបវារៈ" >
              <div class="p-4 md:p-6">
                <div class="p-4 bg-blue-50 rounded-lg border border-blue-100 mb-6">
                  <h3 class="font-moul text-md text-blue-800 mb-2">កម្មវត្ថុ</h3>
                  <p class="text-gray-700 leading-relaxed">{{ record.objective || 'មិនបានកំណត់' }}</p>
                </div>
                <div class="flex justify-between items-center mb-4">
                  <h3 class="font-moul text-lg text-gray-800">តារាងសកម្មភាព</h3>
                ...
                <div v-if="agendas.length===0" class="text-center text-gray-400 py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                ...
                <div v-else class="space-y-3">
                  <div v-for="(item,index) in agendas" :key="item.id||index" class="flex group">
                    ...
                    <div class="flex-none flex flex-col items-center mr-3"><div class="w-3 h-3 rounded-full bg-blue-500 border-2 border-white shadow"></div><div v-if="index<agendas.length-1" class="w-0.5 flex-1 bg-blue-200"></div></div>
                    <div class="flex-1 pb-4">
                      <div class="bg-white rounded-lg border border-gray-200 p-3 hover:border-blue-300 hover:shadow-sm transition-all">
                        <div class="flex items-start justify-between">
                          <div class="flex-1 min-w-0">
                            <div class="flex items-center space-x-2 mb-1"><span class="text-xs font-bold text-gray-400">{{ index+1 }}</span><h4 class="font-bold text-gray-800 text-sm">{{ item.topic }}</h4></div>
                            <span class="inline-flex items-center text-xs text-gray-500 bg-gray-100 rounded-full px-2 py-0.5 mr-2"><svg class="w-3 h-3 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12S6.477 2 12 2zm0 1.5a8.5 8.5 0 1 0 0 17 8.5 8.5 0 0 0 0-17zM11.75 6a.75.75 0 0 1 .75.75V12h3.75a.75.75 0 0 1 0 1.5h-4.5a.75.75 0 0 1-.75-.75V6.75a.75.75 0 0 1 .75-.75z" fill="currentColor"></path></g></svg>{{ item.duration }} នាទី</span>
                            <div v-if="item.handlers&&item.handlers.length>0" class="mt-2 flex flex-wrap items-center gap-1"><svg class="w-3.5 h-3.5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M12 2a4 4 0 1 0 0 8 4 4 0 0 0 0-8zM6 7a6 6 0 1 1 12 0A6 6 0 0 1 6 7zM3.5 19v-.5C3.5 15.693 7.307 14 12 14s8.5 1.693 8.5 4.5v.5a1 1 0 0 1-1 1h-15a1 1 0 0 1-1-1z" fill="currentColor"></path></g></svg><span v-for="(h,hi) in item.handlers" :key="hi" class="text-xs text-blue-700 bg-blue-50 rounded-full px-2 py-0.5">{{ h.name }}<span v-if="h.role" class="text-gray-400"> · {{ h.role }}</span></span></div>
                          </div>
                ...
                <div v-if="agendas.length>0" class="mt-4 pt-4 border-t border-gray-200 flex items-center justify-end"><span class="text-sm text-gray-500 mr-2">សរុបរយៈពេល:</span><span class="font-bold text-blue-700">{{ totalDuration }} នាទី</span><span class="text-gray-400 ml-1">({{ formatDuration(totalDuration) }})</span></div>
                <div v-if="record.summary" class="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-100"><h3 class="font-moul text-md text-gray-800 mb-2">សង្ខេបបឋម</h3><p class="text-gray-700 leading-relaxed">{{ record.summary }}</p></div>
              </div>
            </n-tab-pane>
            ...
            <n-tab-pane name="participants" tab="តារាងអង្គប្រជុំ">
              <div class="p-4 md:p-6">
                ...
                <div v-if="seatLoading" class="flex justify-center py-12 bg-gray-100 rounded-lg border-2 border-gray-300 mb-8">
                  <n-spin size="small" />
                </div>
                <!-- Dynamic seat layout from room seats -->
                <div v-else class="bg-gray-100 rounded-lg border-2 border-gray-300 p-6 mb-8 overflow-x-auto">
                  <div class="text-center text-gray-400 text-xs mb-4 font-bold">ទ្វារចូល</div>
                  <div class="w-20 h-1 bg-gray-300 mx-auto mb-6 rounded"></div>

                  <!-- No seats configured -->
                  <div v-if="roomSeats.length === 0" class="text-center text-gray-400 py-8 border-2 border-dashed border-gray-300 rounded-lg mb-4">
                  ...
                  <div v-else class="relative mx-auto" style="max-width:800px">
                    ...
                    <div v-if="headTableSeats.length > 0" class="mb-6">
                      <div class="text-xs text-gray-500 mb-2 font-semibold text-center">តុប្រធាន</div>
                      <div class="flex justify-center gap-2 flex-wrap">
                        <div v-for="s in headTableSeats" :key="'ht'+s.id" class="flex flex-col items-center">
                          <div :class="['w-12 h-10 rounded-t-lg border-2 border-b-0 text-xs flex items-center justify-center font-bold', s.occupied ? seatColorClass(s.role) : 'bg-white border-gray-300 text-gray-400']"
                               :title="s.occupied ? s.name : ('កៅអី ' + s.seat_number)">
                            {{ s.occupied ? s.label : s.seat_number }}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div v-if="leftSideSeats.length > 0 || rightSideSeats.length > 0" class="flex items-stretch gap-2">
                      <!-- Left side seats (first half of each side row) -->
                      <div v-if="leftSideSeats.length > 0" class="flex flex-col gap-2 justify-center">
                        <div v-for="s in leftSideSeats" :key="'ls'+s.id" class="flex items-center">
                          <div :class="['w-10 h-8 rounded-l-lg border-2 border-r-0 text-xs flex items-center justify-center font-bold', s.occupied ? seatColorClass(s.role) : 'bg-white border-gray-300 text-gray-400']"
                               :title="s.occupied ? s.name : ('កៅអី ' + s.seat_number)">
                            {{ s.occupied ? s.label : s.seat_number }}
                          </div>
                        </div>
                      ...
                      <div class="flex-1 border-2 border-dashed border-amber-400 rounded bg-amber-50/50 flex items-center justify-center min-h-[120px]">
                        <span class="text-amber-600 text-xs font-moul opacity-50">តុប្រជុំ</span>
                      </div>
                      <!-- Right side seats (second half of each side row) -->
                      <div v-if="rightSideSeats.length > 0" class="flex flex-col gap-2 justify-center items-end">
                        <div v-for="s in rightSideSeats" :key="'rs'+s.id" class="flex items-center">
                          <div :class="['w-10 h-8 rounded-r-lg border-2 border-l-0 text-xs flex items-center justify-center font-bold', s.occupied ? seatColorClass(s.role) : 'bg-white border-gray-300 text-gray-400']"
                               :title="s.occupied ? s.name : ('កៅអី ' + s.seat_number)">
                            {{ s.occupied ? s.label : s.seat_number }}
                          </div>
                        </div>
                    ...
                    <div v-if="audienceSeats.length > 0" class="mt-6">
                      <div class="text-xs text-gray-500 mb-2 font-semibold text-center">ទស្សនិកជន</div>
                      <div class="grid gap-2 justify-center" :style="{ gridTemplateColumns: 'repeat(' + audienceCols + ', 64px)' }">
                        <div v-for="s in audienceSeats" :key="'aud'+s.id" class="flex flex-col items-center">
                          <div :class="['w-14 h-10 rounded border-2 text-xs flex items-center justify-center font-bold', s.occupied ? seatColorClass(s.role) : 'bg-white border-gray-300 text-gray-400']"
                               :title="s.occupied ? s.name : ('កៅអី ' + s.seat_number)">
                            {{ s.occupied ? s.label : s.seat_number }}
                          </div>
                        </div>
                  ...
                  <div class="flex flex-wrap gap-4 justify-center mt-6 text-xs">
                    ...
                    <div class="flex items-center"><div class="w-3 h-3 bg-blue-100 border border-blue-400 rounded mr-1"></div>អនុប្រធាន</div>
                    <div class="flex items-center"><div class="w-3 h-3 bg-green-100 border border-green-400 rounded mr-1"></div>សមាជិក</div>
                    <div class="flex items-center"><div class="w-3 h-3 bg-white border border-gray-300 rounded mr-1"></div>ទំនេរ</div>
                  </div>
                </div>

                <h3 class="font-moul text-lg text-gray-800 mb-4">បញ្ជីអ្នកចូលរួម ({{ record.listMembers?record.listMembers.length:0 }} នាក់)</h3>
                ...
                <div v-else class="overflow-x-auto">
                  <table class="w-full border-collapse">
                    <thead><tr class="bg-gray-100 text-left"><th class="p-3 font-moul text-sm border-b border-gray-200 w-12">ល.រ</th><th class="p-3 font-moul text-sm border-b border-gray-200 w-16">កៅអី</th><th class="p-3 font-moul text-sm border-b border-gray-200">ឈ្មោះ</th><th class="p-3 font-moul text-sm border-b border-gray-200">តួនាទី</th><th class="p-3 font-moul text-sm border-b border-gray-200">ក្រុម</th><th class="p-3 font-moul text-sm border-b border-gray-200">វត្តមាន</th></tr></thead>
                    <tbody>
                      <tr v-for="(lm,index) in record.listMembers" :key="lm.id||index" class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        <td class="p-3 text-gray-600">{{ index+1 }}</td>
                        <td class="p-3"><span v-if="lm.seat" class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold" :class="seatBadgeClass(lm)">{{ lm.seat }}</span><span v-else class="text-gray-300">-</span></td>
                        <td class="p-3"><div class="font-bold text-gray-800"><template v-if="lm.member"><template v-if="lm.member.officers&&lm.member.officers.length">{{ lm.member.officers.map(o=>(o.countesy?o.countesy.name+' ':'')) }}{{ lm.member.lastname }} {{ lm.member.firstname }}</template><template v-else>{{ lm.member.lastname }} {{ lm.member.firstname }}</template></template><template v-else>មិនស្គាល់</template></div><div v-if="lm.member&&lm.member.organizations&&lm.member.organizations.length" class="text-xs text-gray-500">{{ lm.member.organizations.map(o=>o.name).join(', ') }}</div></td>
                        <td class="p-3"><span :class="roleClass(lm.role)" class="px-2 py-0.5 rounded text-xs font-bold">{{ roleLabel(lm.role) }}</span></td>
                        <td class="p-3"><span :class="groupClass(lm.group)" class="px-2 py-0.5 rounded text-xs font-bold">{{ groupLabel(lm.group) }}</span></td>
                        <td class="p-3"><span v-if="lm.attendant" class="text-green-600 font-bold text-xs"><template v-if="lm.attendant.people_id==lm.member.id">មកផ្ទាល់</template><template v-else>ជំនួសដោយ {{ lm.attendant.member?lm.attendant.member.lastname+' '+lm.attendant.member.firstname:'' }}</template></span><span v-else class="text-red-500 text-xs">អវត្តមាន</span></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <n-tab-pane name="references" tab="ឯកសារយោង">
              <div class="p-4 md:p-6">
                <div class="flex" style="height: 1024px;">
                  ...
                  <div class="w-72 flex-none border border-gray-200 rounded-l-lg bg-gray-50 overflow-y-auto overflow-x-hidden">
                    <!-- Minister Preeng -->
                    <div class="border-b border-gray-200">
                      ...
                      <div v-for="(f,i) in docSections.ministerPreeng.files" :key="'m'+i"
                        class="flex items-center justify-between px-3 py-1.5 border-b border-gray-100 hover:bg-white cursor-pointer transition-colors"
                    ...
                    <div class="border-b border-gray-200">
                      <div class="flex items-center justify-between px-3 py-2 bg-green-50 border-b border-green-100 sticky top-0 z-10">
                        <div class="flex items-center space-x-1.5">
                          ...
                          <span class="font-bold text-xs text-green-800">របាយការណ៍ទូទៅ</span>
                          <span class="text-xs text-green-400">({{ docSections.reports.files.length }})</span>
                        </div>
                      ...
                      <div v-for="(f,i) in docSections.reports.files" :key="'r'+i"
                        ...
                        <div class="flex items-center space-x-2 min-w-0">
                          <!-- <svg class="w-3.5 h-3.5 text-green-400 flex-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M6.75 3.5A3.25 3.25 0 0 0 3.5 6.75v10.5a3.25 3.25 0 0 0 3.25 3.25h10.5a3.25 3.25 0 0 0 3.25-3.25V9.372a2.25 2.25 0 0 0-.659-1.59L15.218 3.16A2.25 2.25 0 0 0 13.628 2.5H11.5v2.25c0 1.519-1.231 2.75-2.75 2.75H6.75V3.5z" fill="currentColor"></path></g></svg> -->
                          <span class="text-xs text-gray-700 truncate">{{ f.name }}</span>
                        </div>
                  ...
                  <div class="flex-1 border border-gray-200 rounded-r-lg border-l-0 bg-gray-100 flex items-center justify-center overflow-hidden ">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <div class="px-4 py-3 bg-amber-50 border-b border-amber-200 flex items-center justify-between">
                      ...
                      <span class="text-xs font-bold px-2 py-0.5 rounded-full" :class="preCheckAllDone?'bg-green-100 text-green-700':'bg-amber-100 text-amber-700'">{{ preCheckDoneCount }}/{{ preChecklist.length }}</span>
                    </div>
                    <div class="p-4 space-y-1">
                      ...
                      <div v-for="item in preChecklist" :key="'pre'+item.id" class="flex items-center space-x-2 p-2 rounded hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                          <span :class="item.is_checked?'text-gray-400 line-through':'text-gray-700'" class="text-sm font-medium">{{ item.item_name }}</span>
                        </div>
                        <n-tag
                  ...
                  <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    <div class="px-4 py-3 bg-blue-50 border-b border-blue-200 flex items-center justify-between">
                      ...
                      <span class="text-xs font-bold px-2 py-0.5 rounded-full" :class="postCheckAllDone?'bg-green-100 text-green-700':'bg-blue-100 text-blue-700'">{{ postCheckDoneCount }}/{{ postChecklist.length }}</span>
                    </div>
                    <div class="p-4 space-y-1">
            ...
            <n-tab-pane name="draft" tab="សេចក្តីព្រាង">
              <div class="p-4 md:p-6">
                <div v-if="!legalDraft" class="text-center text-gray-400 py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300"><svg class="w-16 mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M6.75 3.5A3.25 3.25 0 0 0 3.5 6.75v10.5a3.25 3.25 0 0 0 3.25 3.25h10.5a3.25 3.25 0 0 0 3.25-3.25V9.372a2.25 2.25 0 0 0-.659-1.59L15.218 3.16A2.25 2.25 0 0 0 13.628 2.5H11.5v2.25c0 1.519-1.231 2.75-2.75 2.75H6.75V3.5z" fill="currentColor"></path></g></svg><div>មិនទាន់មានសេចក្តីព្រាង</div></div>
                <div v-else class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <div class="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-200">
                    <div class="flex items-center space-x-3">
                      <div class="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center"><svg class="w-6 h-6 text-blue-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g fill="none"><path d="M8.75 11.5a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5zm0 2.75a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5zm0 2.75a.75.75 0 0 0 0 1.5h6.5a.75.75 0 0 0 0-1.5h-6.5z" fill="currentColor"></path></g></svg></div>
                      <div>
                        <div class="font-bold text-gray-800">{{ legalDraft.title }}</div>
                        <div class="text-xs text-gray-500">v{{ legalDraft.version_number || 1 }} · {{ legalDraft.regulator || '' }}</div>
                      </div>
                    </div>
                    <div class="flex items-center space-x-2">
                      <router-link :to="{name:'DraftViewer',params:{meeting_id:record.id,draft_id:record.id},query:{meeting_status:record.status,meeting_date:record.date}}" class="inline-flex items-center text-sm text-blue-600 hover:text-blue-800 font-bold px-3 py-1.5 rounded border border-blue-200 hover:border-blue-400 transition-colors">
                      ...
                      <router-link :to="{name:'DraftTimeline',params:{meeting_id:record.id}}" class="inline-flex items-center text-sm text-gray-600 hover:text-gray-800 font-bold px-3 py-1.5 rounded border border-gray-200 hover:border-gray-300 transition-colors">
                  ...
                  <div class="p-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div><span class="text-gray-500">ស្ថានភាព៖</span> <span class="font-bold ml-1" :class="legalDraft.status==='final'?'text-green-600':'text-blue-600'">{{ legalDraft.status==='final'?'ចុងក្រោយ':'កំពុងដំណើរការ' }}</span></div>
                      <div><span class="text-gray-500">កំណែបច្ចុប្បន្ន៖</span> <span class="font-bold ml-1">v{{ legalDraft.version_number || 1 }}</span></div>
                      <div><span class="text-gray-500">គតិយុត្តយោង៖</span> <span class="font-bold ml-1">{{ legalDraft.regulator || 'មិនមាន' }}</span></div>
                    </div>
...
export default {
  ...
  setup(props){
    ...
    function roleClass(r){const c={leader:'bg-yellow-100 text-yellow-700',deputy_leader:'bg-blue-100 text-blue-700',member:'bg-gray-100 text-gray-700'};return c[r]||'bg-gray-100 text-gray-700'}
    ...
    function groupClass(g){const c={lead_meeting:'bg-indigo-100 text-indigo-700',defender:'bg-green-100 text-green-700',audient:'bg-gray-100 text-gray-700'};return c[g]||'bg-gray-100 text-gray-700'}
    ...
    const availableHandlers=computed(()=>{if(!record.value||!record.value.listMembers)return[];return record.value.listMembers.filter(lm=>lm.member).map(lm=>{const n=(lm.member.lastname||'')+' '+(lm.member.firstname||'');return{label:n.trim()+' ('+groupLabel(lm.group)+')',value:lm.member.id,member:lm.member,group:lm.group}})})
    ...
    function saveAgenda(){if(!agendaForm.topic||!agendaForm.start_time||!agendaForm.duration)return;const handlers=agendaForm.handlerIds.map(id=>{const opt=availableHandlers.value.find(h=>h.value===id);return opt?{id:opt.value,name:opt.label.replace(/\s*\(.*\)\s*$/,''),role:groupLabel(opt.group)}:{id,name:'មិនស្គាល់',role:''}});const p={topic:agendaForm.topic.trim(),start_time:agendaForm.start_time,duration:parseInt(agendaForm.duration),handlers};if(editingAgendaIndex.value!==null){agendas.value[editingAgendaIndex.value]={...agendas.value[editingAgendaIndex.value],...p}}else{agendas.value.push({id:Date.now(),...p});agendas.value.sort((a,b)=>(a.start_time||'').localeCompare(b.start_time||''))};showAgendaForm.value=false}
    function removeAgenda(i){agendas.value.splice(i,1)}
    function hydrateAgendas(){if(record.value&&record.value.agendas&&Array.isArray(record.value.agendas)){agendas.value=[...record.value.agendas].sort((a,b)=>(a.start_time||'').localeCompare(b.start_time||''))}else{agendas.value=[]}}

    // ─── Seating (Dynamic from room seats) ──────────────────────────────
    const roomSeats = ref([])
    ...
    function enrichSeat(seat) {
      ...
      const lm = record.value?.listMembers?.find(m => String(m.seat) === seatNum)
      if (lm && lm.member) {
        const name = (lm.member.lastname || '') + ' ' + (lm.member.firstname || '')
        return { ...seat, occupied: true, label: name.charAt(0) || '?', name: name.trim(), role: lm.role, member: lm.member }
      }
      return { ...seat, occupied: false, label: '', name: '', role: '' }
    }
    ...
    function seatColorClass(role){if(role==='leader')return'bg-yellow-100 border-yellow-500 text-yellow-700';if(role==='deputy_leader')return'bg-blue-100 border-blue-400 text-blue-700';return'bg-green-100 border-green-400 text-green-700'}
    function seatBadgeClass(lm){if(!lm)return'bg-gray-100 text-gray-400';if(lm.role==='leader')return'bg-yellow-100 text-yellow-700 border border-yellow-300';if(lm.role==='deputy_leader')return'bg-blue-100 text-blue-700 border border-blue-300';return'bg-green-100 text-green-700 border border-green-300'}
    ...
    function fetchMeeting(){
      ...
      store.dispatch('meeting/read',{id:mid}).then(res=>{
        if(res.data&&res.data.record){record.value=res.data.record}else{record.value=getMockRecord()}
    ...
    return{record,loading,error,formatDate,meetingLeaders,roleLabel,roleClass,groupLabel,groupClass,
      ...
      roomSeats,seatLoading,headTableSeats,leftSideSeats,rightSideSeats,audienceSeats,audienceCols,seatColorClass,seatBadgeClass,
