  ...
  <div class="actions z-40">
      ...
      <div v-if="show" class="panel">
          ...
          <template #trigger>
            <div class="action-board">
              <svg
                ...
                @click="showOthersDocumentModal(record)"
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            <div class="action-board">
              <svg
                class="action text-blue-500"
                @click="showPrengModal(record)"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 24 24"
              >
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            <div class="action-board">
              <svg
                class="action text-blue-500"
                @click="$router.push('/meetings/' + record.id)"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 24 24"
              >
              ...
              <div class="action-label">លម្អិត</div>
            </div>
          </template>
          លម្អិត
        </n-tooltip>
        <n-tooltip trigger="hover">
          <template #trigger>
            <div class="action-board">
              <svg
                class="action text-red-500"
                @click="showMinisterPrengModal(record)"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            <div class="action-board">
              <svg
                class="action text-red-500"
                @click="showRegulatorModal(record)"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 1024 1024"
              >
          ...
          <template #trigger>
            <div class="action-board">
              <svg
                ...
                :title="
                  record.active == 1
                    ? 'គណនីនេះកំពុងបើកតំណើរការ'
                    : 'គណនីនេះកំពុងត្រូវបានបិទមិនអាចប្រើប្រាស់បាន'
                "
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 24 24"
        ...
        <n-tooltip
          v-if="
            user != null && user != undefined && user.id == record.created_by
          "
          trigger="hover"
        >
          <template #trigger>
    ...
    <n-tooltip v-if="!show" trigger="hover">
      <template #trigger>
        <div
          :class="
            ' record-status text-xxs font-bold ' +
            statuses.find((s) => s.value == record.status).color
          "
        >
    ...
    <pdf-preview-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="pdfPreviewModal.show"
    ...
    <regulator-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="regulatorModal.show"
      :onClose="closeRegulator"
    />
    <!-- Form preng -->
    <preng-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="prengModal.show"
      :onClose="closePreng"
    />
    <!-- Form minister preng -->
    <minister-preng-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="ministerPrengModal.show"
      :onClose="closeMinisterPreng"
    />
    <!-- Form others document -->
    <others-document-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="othersDocumentModal.show"
      :onClose="closeOthersDocument"
    />
    <!-- Form others document -->
    <report-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="reportModal.show"
      :onClose="closeReport"
    />
    <!-- Form others document -->
    <tech-report-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="techReportModal.show"
      :onClose="closeTechReport"
    />
    <!-- Form member account -->
    <member-form
      v-bind:model="model"
      v-bind:record="record"
      v-bind:show="memberModal.show"
      :onClose="closeMember"
    />
    <!-- Form room -->
    <room-form
...
export default {
  ...
  setup(props) {
    ...
    function toggleActive(record) {
      ...
      store
        .dispatch(props.model.name + "/toggleActive", {
          id: record.id,
        })
        .then((res) => {
          if (res.data.ok) {
