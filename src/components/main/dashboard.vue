<template>
  <div class="ocm-dashboard relative">
    <!-- Window bar -->
    <WindowBar :title="model.title" :icon="4" />
    ...
    <!-- Dashboard content -->
    <div v-else class="dashboard-content mt-12 p-4">
      <!-- Section: Summary Cards -->
      <div class="mb-6">
        <div class="font-moul text-sm text-gray-600 mb-3 px-2">សង្ខេបទិន្នន័យ</div>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div @click="router.push('/users')"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="text-xs text-gray-500">គណនីអ្នកប្រើប្រាស់</div>
          ...
          <div @click="router.push('/meetings')"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="text-xs text-gray-500">កិច្ចប្រជុំ</div>
          ...
          <div @click="router.push('/folders')"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="text-xs text-gray-500">ឯកសារគតិយុត្ត</div>
          ...
          <div @click="router.push('/tasks')"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="text-xs text-gray-500">កិច្ចការថ្មី</div>
          ...
          <div @click="router.push('/tasks')"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="text-xs text-gray-500">កំពុងអនុវត្ត</div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <div class="font-moul text-sm text-gray-600 mb-3">កិច្ចប្រជុំតាមប្រភេទ</div>
          <div v-if="meetingsByType.length === 0" class="text-center text-gray-400 py-8 text-sm">
            មិនមានទិន្នន័យ
          </div>
          <div v-else class="space-y-2">
            <div v-for="(item, idx) in meetingsByType" :key="idx"
              class="flex items-center gap-2">
              <div class="w-32 text-xs text-gray-600 truncate" :title="item.name || item.type">
              ...
              <div class="text-xs font-semibold text-gray-700 w-10 text-right">{{ item.count }}</div>
            </div>
          </div>
        </div>
        ...
        <!-- Meetings by Status -->
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <div class="font-moul text-sm text-gray-600 mb-3">ស្ថានភាពកិច្ចប្រជុំ</div>
          <div v-if="meetingsByStatus.length === 0" class="text-center text-gray-400 py-8 text-sm">
            មិនមានទិន្នន័យ
          </div>
          <div v-else class="space-y-2">
            <div v-for="(item, idx) in meetingsByStatus" :key="idx"
              class="flex items-center gap-2">
              <div class="w-32 text-xs text-gray-600 truncate" :title="item.name || item.status">
      ...
      <div class="mb-6" v-if="meetingsByOrganization.length > 0">
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <div class="font-moul text-sm text-gray-600 mb-3">កិច្ចប្រជុំតាមស្ថាប័ន</div>
          <div class="space-y-2">
            <div v-for="(item, idx) in meetingsByOrganization.slice(0, 8)" :key="idx"
              class="flex items-center gap-2">
              <div class="w-40 text-xs text-gray-600 truncate" :title="item.name || item.organization">
      ...
      <div class="mb-6">
        <div class="font-moul text-sm text-gray-600 mb-3 px-2">ច្រកចូលរហ័ស</div>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
          <div v-for="link in quickLinks" :key="link.url"
            ...
            <div class="text-xs font-pvh text-center text-gray-600" v-html="link.name"></div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    /** Translate status codes to Khmer labels */
...
.quick-link-card {
  @apply bg-white rounded-lg border border-gray-200 p-4 text-center hover:border-blue-300 hover:shadow-md transition-all duration-200 cursor-pointer;
}
